...
class ARKWorkflowTests(unittest.TestCase):
    ...
    def test_solve_ark_problem_reports_jacobian_callback_shape_errors(self) -> None:
        def rhs(_t: float, y: np.ndarray) -> np.ndarray:
            return -y

        def bad_jac(t: float, _y: np.ndarray) -> np.ndarray:
            if t <= 1e-15:
                return np.array([[-1.0]], dtype=np.float64)
            return np.array([[-1.0, 0.0]], dtype=np.float64)

        problem = ARKProblem(
            name="ark_bad_jac_shape",
            dimension=1,
            initial_time=0.0,
            initial_state=np.array([1.0], dtype=np.float64),
            rhs=rhs,
            jacobian=bad_jac,
            method=DIRK_BACKWARD_EULER,
        )
